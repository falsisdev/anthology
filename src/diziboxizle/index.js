...
function cfgReady() {
    if (!_cfgReady) {
        _cfgReady = loadConfig().then(function () {
            var v;
            v = val('urls.series.diziboxizle.base'); if (v) BASE_URL = String(v).replace(/\/+$/, '');
...
/**
 * Anthology - DiziBoxİzle Provider
 * https://diziboxizle.com/ yabancı dizi arşivi
 * VidMoly (master.m3u8) ve Ok.ru (1080p MP4/HLS) doğrudan akışları sağlar.
 */
...
var SERIES_MAP = {
  "56-days": "56 Days",
  "angela-diniz-assassinada-e-condenada": "Ângela Diniz: Assassinada e Condenada",
  ...
  "dear-life": "Dear Life",
  "det-som-goms-i-sno": "Det som göms i snö",
  ...
  "the-rain-in-espana": "The Rain in España",
...
/**
 * TMDB / IMDb ID'den çok dilli dizi başlıklarını toplar
 */
async function resolveTmdbInfo(id, mediaType) {
  try {
    ...
    if (numericId) {
      ...
      var enRes = await fetch('https://api.themoviedb.org/3/' + type + '/' + numericId + '?api_key=' + TMDB_API_KEY + '&language=en-US', { signal: timeoutSignal(6000) });
      if (enRes.ok) {
    ...
    return { titles: titles, numericId: numericId };
  } catch (e) {
...
/**
 * Site içi arama yapar (Wordpress REST API ve HTML arama)
 */
async function searchSeries(query) {
  try {
    ...
    // 1. Önce SERIES_MAP eşleşmesi kontrol et
    for (var slug in SERIES_MAP) {
      if (Object.prototype.hasOwnProperty.call(SERIES_MAP, slug)) {
        ...
        if (sClean === cleanQ || slugClean === cleanQ || sClean.indexOf(cleanQ) !== -1 || cleanQ.indexOf(sClean) !== -1) {
          seenSlugs[slug] = true;
          results.push({ slug: slug, title: sTitle, url: BASE_URL + '/' + slug + '/' });
        }
      }
    }

    // 2. WP REST API Categories arama
    try {
      var wpCatUrl = BASE_URL + '/wp-json/wp/v2/categories?search=' + encodeURIComponent(query);
      var wpRes = await fetch(wpCatUrl, { headers: HEADERS, signal: timeoutSignal(6000) });
      if (wpRes.ok) {
        var wpCats = await wpRes.json();
    ...
    // 3. HTML Arama Sayfası
    try {
      var searchUrl = BASE_URL + '/?s=' + encodeURIComponent(query);
      var sRes = await fetch(searchUrl, { headers: HEADERS, signal: timeoutSignal(6000) });
      if (sRes.ok) {
...
/**
 * Adaylar arasından en uygun diziyi seçer
 */
...
/**
 * Dizi sayfasından bölüm bağlantılarını çeker
 */
function parseEpisodes(detailHtml) {
  ...
  while ((m = epRegex.exec(detailHtml)) !== null) {
    ...
    if (!seen[key]) {
      ...
      episodes.push({
        url: url,
        title: rawText || (season + '. Sezon ' + episode + '. Bölüm'),
        season: season,
        episode: episode
      });
    }
...
/**
 * Hedef sezon ve bölümü eşleştirir
 */
...
/**
 * VidMoly HLS (Master Playlist) Çözücü
 */
...
/**
 * Bölüm sayfasındaki iframe ve oynatıcı kaynaklarını çıkarıp çözer
 */
  ...
  try {
    ...
    var promises = iframes.map(async function(src) {
      if (src.indexOf('vidmoly') !== -1) {
        return await resolveVidMoly(src);
...
/**
 * getCatalog: Dizi arşivi listesini döner
 */
async function getCatalog(args) {
  try {
    ...
    if (query) {
      var searchList = await searchSeries(query);
      ...
      if (linkMatch && titleMatch) {
        ...
        if (!seen[slug]) {
          ...
          metasArr.push({
            ...
            description: descMatch ? decodeHtmlEntities(descMatch[1].replace(/<[^>]+>/g, '').trim()) : undefined,
            imdbRating: imdbMatch ? imdbMatch[1] : undefined,
            posterShape: 'poster'
          });
        }
      }
...
/**
 * getMeta: Dizi detaylarını ve bölüm listesini döner
 */
async function getMeta(args) {
  try {
    ...
    var poster = ogImage ? ogImage[1] : undefined;
    var description = ogDesc ? decodeHtmlEntities(ogDesc[1].trim()) : undefined;
...
/**
 * getStreams: TMDB / IMDb ID veya dahili diziboxizle:ep:* ID ile video akışlarını döner
 */
async function getStreams(tmdbId, mediaType, seasonNum, episodeNum) {
  try {
    ...
    // 2. Doğrudan Bölüm ID'si (diziboxizle:ep:*)
    if (typeof tmdbId === 'string' && tmdbId.startsWith('diziboxizle:ep:')) {
      var epSlug = tmdbId.replace('diziboxizle:ep:', '');
      return await fetchEpisodeMirrors(BASE_URL + '/' + epSlug + '/');
    }

    // 3. Doğrudan Dizi ID'si (diziboxizle:show:*)
    ...
    if (typeof tmdbId === 'string' && tmdbId.startsWith('diziboxizle:show:')) {
      ...
      if (showMeta && showMeta.meta && Array.isArray(showMeta.meta.videos) && showMeta.meta.videos.length > 0) {
        ...
        return await getStreams(matchedV.id, mediaType, finalSeason, finalEpisode);
      }
    }

    // 4. TMDB / IMDb ID Formatı (tt123456:1:1 veya numeric:1:1)
    ...
    // Dizi detay sayfasından bölümleri çek
