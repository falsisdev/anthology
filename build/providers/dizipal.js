...
function extractFromImagestoo(embedUrl) {
  return __async(this, null, function* () {
    try {
...
function getStreams(tmdbId, mediaType, seasonNum, episodeNum) {
  return __async(this, null, function* () {
    try {
      ...
      let pageRes = yield fetch(targetUrl, { headers: HEADERS });
      if (!pageRes.ok && isTv) {
        const showRes = yield fetch(matchedItem.url, { headers: HEADERS });
        if (showRes.ok) {
          ...
          if (epMatch) {
            ...
            pageRes = yield fetch(targetUrl, { headers: HEADERS });
          }
        }
      }
      if (!pageRes.ok) return [];
      const html = yield pageRes.text();
      const mCfg = html.match(/id=["']videoContainer["'][^>]*data-cfg=["']([^"']+)["']/i) || html.match(/data-cfg=["']([^"']+)["']/i);
      if (!mCfg) return [];
      ...
      if (embedUrl.includes("imagestoo")) {
        const securedLink = yield extractFromImagestoo(embedUrl);
        if (securedLink) {
          streams.push({
            name: displayTitle,
        ...
        try {
          const embRes = yield fetch(embedUrl, {
            headers: { "User-Agent": HEADERS["User-Agent"], "Referer": BASE_URL + "/" }
          });
          if (embRes.ok) {
            const embHtml = yield embRes.text();
            const mFile = embHtml.match(/file:\s*["'](https?:\/\/[^"']+\.m3u8[^"']*)["']/i) || embHtml.match(/https?:\/\/[^"'\s<>]+\.m3u8[^"'\s<>]*/i);
            if (mFile) {
