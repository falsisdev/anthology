...
function fetchWithTimeout(_0) {
  return __async(this, arguments, function* (url, options = {}, timeoutMs = 5e3) {
...
function decodeHtmlEntities(str) {
  if (!str) return "";
  return str.toString().replace(/&quot;/g, '"').replace(/&#039;/g, "'").replace(/&#39;/g, "'").replace(/&amp;/g, "&").replace(/&lt;/g, "<").replace(/&gt;/g, ">").replace(/&nbsp;/g, " ").replace(/&#(\d+);/g, function(match, dec) {
...
function qualityWeight(q) {
  ...
  if (s.includes("2160") || s.includes("4k")) return 4e3;
...
function resolveVidmoly(embedUrl, referer) {
  return __async(this, null, function* () {
    try {
      const fullUrl = embedUrl.startsWith("//") ? "https:" + embedUrl : embedUrl;
      const res = yield fetchWithTimeout(fullUrl, {
        headers: {
...
function resolveKitsu(kitsuId) {
  return __async(this, null, function* () {
    try {
      ...
      const res = yield fetchWithTimeout(`https://kitsu.io/api/edge/anime/${cleanId}`);
      if (!res.ok) return null;
    ...
    if (numericId) {
      ...
      try {
        ...
        if (!dataEn && !dataTr && isTv) {
          const mRes = yield fetchWithTimeout(`https://api.themoviedb.org/3/movie/${numericId}?api_key=${TMDB_API_KEY}&language=en-US&append_to_response=alternative_titles`);
        ...
        if (dataEn) {
          const normalizeTitle = (str) => {
            ...
            return str.replace(/[\u00E0-\u00E5]/gi, "a").replace(/[\u00E8-\u00EB]/gi, "e").replace(/[\u00EC-\u00EF]/gi, "i").replace(/[\u00F2-\u00F6]/gi, "o").replace(/[\u00F9-\u00FC]/gi, "u").replace(/\u00F1/gi, "n").replace(/\u00E7/gi, "c").replace(/[\u0300-\u036f]/g, "").trim();
          };
    ...
    try {
      if (typeof tmdbId === "object" && tmdbId !== null) {
        ...
        return getStreams(
          ...
          mediaType || obj.type,
          seasonNum !== void 0 ? seasonNum : obj.season,
          episodeNum !== void 0 ? episodeNum : obj.episode
        );
      }
      const mediaInfo = yield resolveMediaInfo(tmdbId, mediaType, seasonNum, episodeNum);
      ...
      if (rawId.startsWith("cizgimax:show:")) {
        const showMeta = yield getMeta(rawId);
        if (showMeta && showMeta.meta && Array.isArray(showMeta.meta.videos) && showMeta.meta.videos.length > 0) {
          return yield getStreams(showMeta.meta.videos[0].id, mediaType, seasonNum, episodeNum);
        }
      }
      ...
      if (rawId.startsWith("cizgimax:")) {
        ...
        if (!isTv && mediaInfo.numericId) {
          try {
            const probeRes = yield fetchWithTimeout(`${BASE_URL}/film/film-${mediaInfo.numericId}-izle/`, { headers: HEADERS });
            if (probeRes.ok) {
        ...
        if (!matchedHref) {
          const searchQueries = mediaInfo.queries.length > 0 ? mediaInfo.queries : [mediaInfo.title, mediaInfo.origTitle].filter(Boolean);
          const targetCleans = searchQueries.map((q) => ultraClean(q)).filter(Boolean);
          const seenCleanQueries = /* @__PURE__ */ new Set();
          ...
          for (const q of deduplicatedQueries) {
            try {
              let searchRes = yield fetchWithTimeout(`${BASE_URL}/ara/?q=${encodeURIComponent(q)}`, { headers: HEADERS });
              if (searchRes.status === 429) {
                yield new Promise((r) => setTimeout(r, 1e3));
                searchRes = yield fetchWithTimeout(`${BASE_URL}/ara/?q=${encodeURIComponent(q)}`, { headers: HEADERS });
              }
      ...
      if (isTv && !isDirectEpisode) {
        let detailRes = yield fetchWithTimeout(detailUrl, { headers: HEADERS });
        if (!detailRes.ok) {
          yield new Promise((r) => setTimeout(r, 400));
          detailRes = yield fetchWithTimeout(detailUrl, { headers: HEADERS });
        }
        if (!detailRes.ok) return [];
        const detailHtml = yield detailRes.text();
        let epHref = null;
      ...
      let pageRes = yield fetchWithTimeout(targetPageUrl, { headers: HEADERS });
      if (!pageRes.ok) {
        yield new Promise((r) => setTimeout(r, 400));
        pageRes = yield fetchWithTimeout(targetPageUrl, { headers: HEADERS });
      }
      ...
      for (const server of uniqueServers) {
        let label = server.label || server.type || "\xC7izgiMax";
        ...
        if (server.resolveUrl) {
          try {
            ...
            const rRes = yield fetchWithTimeout(rUrl, {
              headers: {
                "User-Agent": HEADERS["User-Agent"],
            ...
            if (rRes.ok) {
              let rData = null;
              try {
                rData = yield rRes.json();
              } catch (e) {
              }
              if (rData && rData.id) {
                const tauRes = yield fetchWithTimeout(`https://tau-video.xyz/api/video/${rData.id}`);
                if (tauRes.ok) {
                  let tauData = null;
                  try {
                    tauData = yield tauRes.json();
                  } catch (e) {
                  }
                  if (tauData && Array.isArray(tauData.urls)) {
                    for (const u of tauData.urls) {
                      if (u.url && !seenUrls.has(u.url) && !u.url.includes("yhwach.icu")) {
                        ...
                        streams.push({
                          name: `\xC7izgiMax - ${label} [${quality}]`,
                          title: `\xC7izgiMax | ${label} (${quality})`,
                          url: u.url,
                          quality,
                          provider: "cizgimax",
                          headers: strHeaders,
        ...
        if (server.streamUrl) {
          try {
            ...
            const headRes = yield fetchWithTimeout(stUrl, {
              headers: {
                "User-Agent": HEADERS["User-Agent"],
                "Referer": targetPageUrl
              },
            ...
            if (loc) {
              if (loc.includes("vidmoly")) {
                const vRes = yield resolveVidmoly(loc, BASE_URL + "/");
                if (vRes && vRes.url && !seenUrls.has(vRes.url)) {
                  seenUrls.add(vRes.url);
                  streams.push({
                    name: `\xC7izgiMax - ${label} [1080p]`,
                    title: `\xC7izgiMax | ${label} (1080p)`,
                    url: vRes.url,
                    quality: "1080p",
                    provider: "cizgimax",
                ...
                streams.push({
                  name: `\xC7izgiMax - ${label}`,
                  title: `\xC7izgiMax | ${label} [HD]`,
                  url: loc,
                  quality: "HD",
                  provider: "cizgimax",
                  headers: locHeaders,
        ...
        if (server.src) {
          try {
            if (server.src.includes("/oynat/")) {
              ...
              const oRes = yield fetch(oynatUrl, {
                headers: {
                  "User-Agent": HEADERS["User-Agent"],
                  "Referer": targetPageUrl
                },
              ...
              if (loc) {
                if (loc.includes("vidmoly")) {
                  ...
                  const sRes = yield resolveSibnet(loc);
              ...
              const vRes = yield resolveVidmoly(server.src, BASE_URL + "/");
              if (vRes && vRes.url && !seenUrls.has(vRes.url)) {
                seenUrls.add(vRes.url);
                streams.push({
                  name: `\xC7izgiMax - ${label} [1080p]`,
              ...
              const sibRes = yield resolveSibnet(server.src);
        ...
        if (server.videoId && (server.type === "sibnet" || server.label && server.label.toLowerCase().includes("sibnet"))) {
          try {
            const sibRes = yield resolveSibnet(`https://video.sibnet.ru/shell.php?videoid=${server.videoId}`);
            if (sibRes && sibRes.url && !seenUrls.has(sibRes.url)) {
              seenUrls.add(sibRes.url);
              streams.push({
                name: `\xC7izgiMax - ${label}`,
...
function getCatalog(args) {
  return __async(this, null, function* () {
    try {
      ...
      const res = yield fetch(targetUrl, { headers: HEADERS });
      ...
      while ((m = itemRegex.exec(html)) !== null) {
        ...
        if (hrefMatch && nameMatch) {
          ...
          metas.push({
            ...
            poster,
            background: poster,
            genres: ["\xC7izgi Dizi", "\xC7izgiMax"],
            description: `${title} - \xC7izgiMax Ar\u015Fivi`
          });
        }
      }
      if (metas.length === 0) {
          ...
          if (/class=["'][^"']*film-name[^"']*["']/i.test(attrs)) {
            ...
            if (hrefMatch) {
              ...
              metas.push({
                ...
                genres: ["\xC7izgi Dizi", "\xC7izgiMax"],
                description: `${text} - \xC7izgiMax Ar\u015Fivi`
              });
            }
          }
        }
...
function getMeta(args) {
  return __async(this, null, function* () {
    try {
      ...
      const title = decodeHtmlEntities(titleMatch ? titleMatch[1].replace(/<[^>]+>/g, "").trim() : "\xC7izgiMax");
        ...
        if (/class=["'][^"']*ep-num-btn[^"']*["']/i.test(attrs)) {
          ...
          if (hrefMatch) {
            ...
            videos.push({
              ...
              title: `${season}. Sezon ${episode}. B\xF6l\xFCm`,
              season,
              episode
            });
          }
      ...
      return {
        meta: {
          ...
          poster,
          background: poster,
          description: `${title} - \xC7izgiMax`,
          genres: ["\xC7izgi Dizi", "\xC7izgiMax"],
          videos: videos.length > 0 ? videos : [{ id: rawId, title: `${title} 1. B\xF6l\xFCm`, season: 1, episode: 1 }]
        }
      };
    } catch (e) {
      return { meta: null };
