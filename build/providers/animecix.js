...
var tkNormalizeMap = { a: 224 | 0, e: 232 | 0, i: 236 | 0, o: 242 | 0, u: 249 | 0, n: 241 | 0 };
function asciiFold(str) {
  if (!str) return "";
  var s = str.toString();
  if (typeof s.normalize === "function") {
    try {
      return s.normalize("NFD").replace(/[\u0300-\u036f]/g, "").replace(/[^a-z0-9]/gi, "");
    } catch (e) {
    }
  }
  var out = "";
  for (var i = 0; i < s.length; i++) {
    ...
    if (/[\u00E0-\u00FF]/.test(lower)) {
      ...
      var base = lower;
      if (code >= 224 && code <= 229) base = "a";
      else if (code >= 232 && code <= 235) base = "e";
      else if (code >= 236 && code <= 239) base = "i";
      else if (code >= 242 && code <= 246) base = "o";
      else if (code >= 249 && code <= 252) base = "u";
      else if (code === 241) base = "n";
      else if (code === 231) base = "c";
      out += base;
  ...
  return __async(this, null, function* () {
    try {
      ...
      if (typeof tmdbId === "string" && tmdbId.startsWith("animecix:ep:")) {
        ...
        const bestRes2 = yield fetch(videoUrl2, { headers: HEADERS, redirect: "follow", signal: timeoutSignal(5e3) });
        const finalUrl = bestRes2.url || "";
      ...
      for (const q of queries.slice(0, 10)) {
        try {
          const searchRes = yield fetch(`${BASE_URL}/secure/search/${encodeURIComponent(q)}?limit=15`, { headers: HEADERS, signal: timeoutSignal(5e3) });
          ...
          const subTitle = results.filter((r) => {
            const rClean = ultraClean(r.name || "");
            const rjClean = ultraClean(r.name_romanji || "");
            const enClean = ultraClean(r.name_english || "");
            if (/liveaction/i.test(rClean + rjClean + enClean)) return false;
            return qClean.length >= 4 && (rClean.includes(qClean) || rjClean.includes(qClean) || enClean.includes(qClean) || qClean.includes(rClean));
          }).sort((a, b) => {
      ...
      if (isMovie) {
        try {
          const dRes = yield fetch(`${BASE_URL}/secure/titles/${matchedItem.id}`, { headers: HEADERS, signal: timeoutSignal(5e3) });
          if (dRes.ok) {
            ...
            for (const v of videos) {
              if (v.url) {
                ...
                if (tauM) {
                  try {
                    const tauRes = yield fetch(`https://tau-video.xyz/api/video/${tauM[1]}`, {
                      headers: { "User-Agent": HEADERS["User-Agent"], "Referer": BASE_URL + "/" },
                      signal: timeoutSignal(5e3)
                    });
                    if (tauRes.ok) {
                ...
                if (v.url.includes("vidmoly")) {
                  const vm = yield resolveVidmoly(v.url, BASE_URL + "/");
                  if (vm) {
                    return [{
                      name: displayTitle,
      ...
      const bestRes = yield fetch(videoUrl, { headers: HEADERS, redirect: "follow", signal: timeoutSignal(5e3) });
      if (bestRes.ok) {
