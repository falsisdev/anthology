/**
 * Anthology - Tranimeizle Provider
 * Anime Dizi ve Film destekler
 */

var BASE_URL = 'https://www.tranimeizle.io';
...
async function getStreams(tmdbId, mediaType, seasonNum, episodeNum) {
  ...
  try {
    ...
    for (const slug of slugs) {
      const targetUrl = isTV ? `${BASE_URL}/anime/${slug}-${s}-sezon-${e}-bolum` : `${BASE_URL}/anime/${slug}`;
      try {
        ...
        while ((m = videoRe.exec(html)) !== null) {
          ...
          if (src.startsWith('http') && !src.includes('google') && !src.includes('disqus')) {
            streams.push({
              ...
              title: `⌜ Anthology ⌟ | Tranimeizle [HD]`,
              url: src,
              quality: 'HD',
              headers: HEADERS
            });
