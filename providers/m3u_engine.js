...
/**
 * Word-aware title matcher.
 * Returns true when the M3U movie name (before '(' / '-') starts with one of the
 * target titles' token sequences. This prevents short titles like "O" (IT) from
 * matching hundreds of unrelated entries, and keeps "Baba" from matching
 * "Babam ve Oğlum" / "Dağları Deviren Baba".
 */
...
if (typeof searchFilmStreams === "function") {
    var _origSearchFilm = searchFilmStreams;
    searchFilmStreams = async function() {
        var res = await _origSearchFilm.apply(this, arguments);
        return sortStreamsByQuality(res);
    };
}
if (typeof searchDiziStreams === "function") {
    var _origSearchDizi = searchDiziStreams;
    searchDiziStreams = async function() {
        var res = await _origSearchDizi.apply(this, arguments);
        return sortStreamsByQuality(res);
    };
}
