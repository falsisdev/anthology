...
func (p *Provider) GetStreams(ctx context.Context, media models.MediaInfo) ([]models.Stream, error) {
	...
	var showURL string
	doc.Find("article a, .film-card a, .post-title a, .film-content a, a").EachWithBreak(func(i int, s *goquery.Selection) bool {
		href, exists := s.Attr("href")
		if !exists || !strings.HasPrefix(href, BaseURL) || strings.Contains(href, "/giris") || strings.Contains(href, "/kategori") {
			return true
		}
	...
	// Episode URL format: https://www.diziyo.so/{showSlug}-{season}-sezon-{episode}-bolum/
	...
	epDoc.Find("iframe").Each(func(i int, s *goquery.Selection) {
		...
		streams = append(streams, models.Stream{
			Name:     media.Title,
			Title:    fmt.Sprintf("⌜ Diziyo ⌟ | %s", serverName),
			URL:      src,
			Quality:  "1080p",
			Provider: ID,
			Headers: map[string]string{
