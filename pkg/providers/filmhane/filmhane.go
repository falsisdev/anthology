...
const (
	ID      = "filmhane"
	Name    = "Filmhane"
	BaseURL = "https://www.filmhane.shop"
)
...
func (p *Provider) GetStreams(ctx context.Context, media models.MediaInfo) ([]models.Stream, error) {
	...
	doc.Find("article a, .film-card a, .poster a, .post-title a, a").EachWithBreak(func(i int, s *goquery.Selection) bool {
		...
		if !exists || !strings.HasPrefix(href, BaseURL) || strings.Contains(href, "/category/") {
			return true
		}
	...
	if targetURL == "" {
		...
		if media.Type == models.MediaTypeTV {
			targetURL = fmt.Sprintf("%s/dizi/%s-izle", BaseURL, slug)
		} else {
			targetURL = fmt.Sprintf("%s/film/%s-izle", BaseURL, slug)
		}
	}
	...
	targetDoc.Find("iframe").Each(func(i int, s *goquery.Selection) {
		...
		streams = append(streams, models.Stream{
			Name:     media.Title,
			Title:    fmt.Sprintf("⌜ Filmhane ⌟ | %s", serverName),
			URL:      src,
			Quality:  "1080p",
			Provider: ID,
			Headers: map[string]string{
