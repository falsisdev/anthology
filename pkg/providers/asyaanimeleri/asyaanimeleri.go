...
const (
	ID      = "asyaanimeleri"
	Name    = "Asyaanimeleri"
	BaseURL = "https://asyaanimeleri.top"
)
...
func (p *Provider) GetStreams(ctx context.Context, media models.MediaInfo) ([]models.Stream, error) {
	...
	doc.Find("article a, .film-card a, .poster a, .post-title a, a").EachWithBreak(func(i int, s *goquery.Selection) bool {
		...
		if !exists || (!strings.HasPrefix(href, BaseURL) && !strings.HasPrefix(href, "/")) || strings.Contains(href, "/ceviri-istegi") {
			return true
		}
	...
	targetDoc.Find("iframe").Each(func(i int, s *goquery.Selection) {
		...
		serverName := "Asyaanimeleri Player"
		if strings.Contains(src, "vidmoly") {
			serverName = "VidMoly"
		} else if strings.Contains(src, "sibnet") {
			serverName = "Sibnet"
		} else if strings.Contains(src, "rumble") {
			serverName = "Rumble"
		}
		...
		if err == nil && len(extracted) > 0 {
			for _, es := range extracted {
				streams = append(streams, models.Stream{
					Name:     media.Title,
					Title:    fmt.Sprintf("⌜ Asyaanimeleri ⌟ | %s", es.Title),
					URL:      es.URL,
					Quality:  es.Quality,
					Provider: ID,
					Headers:  es.Headers,
			...
			streams = append(streams, models.Stream{
				Name:     media.Title,
				Title:    fmt.Sprintf("⌜ Asyaanimeleri ⌟ | %s", serverName),
				URL:      src,
				Quality:  "1080p",
				Provider: ID,
				Headers: map[string]string{
